...
class RediscoveryTest {
    ...
    //        var expectedComposition =
    //                new ClusterComposition(42, asOrderedSet(A, B, C), asOrderedSet(A, B, C), asOrderedSet(B, E),
    // null);
    //
    ...
    //        var initialRouter = A;
    //        var validComposition = new ClusterComposition(42, asOrderedSet(A), asOrderedSet(A), asOrderedSet(A),
    // null);
    //
    //        Map<BoltServerAddress, Object> responsesByAddress = new HashMap<>();
    //        responsesByAddress.put(initialRouter, validComposition); // initial -> valid composition
    ...
    //
    //        Map<BoltServerAddress, Object> responsesByAddress = new HashMap<>();
    //        responsesByAddress.put(initialRouter, new BoltServiceUnavailableException("Hi")); // initial -> non-fatal
    // error
    ...
    //        Map<BoltServerAddress, Object> responsesByAddress = singletonMap(A, new
    // BoltServiceUnavailableException("Hi!"));
    //        var connectionProviderGetter = connectionProviderGetter(responsesByAddress);
    ...
    //        verify(logger, never())
    //                .log(eq(System.Logger.Level.INFO), startsWith("Unable to fetch new routing table, will try again
    // in "));
    //    }
    //
    //    @Test
    ...
    //        var pool = mock(BoltConnectionSource.class);
    //        given(pool.getConnection(any())).willReturn(failedFuture(new BoltServiceUnavailableException("not
    // available")));
    //        var logging = mock(LoggingProvider.class);
    //        var logger = mock(System.Logger.class);
    //        given(logging.getLog(any(Class.class))).willReturn(logger);
    //        doAnswer(invocationOnMock -> String.format(invocationOnMock.getArgument(0),
    ...
    //    }
    //
    //    private Function<BoltServerAddress, BoltConnectionSource> connectionProviderGetter(
    //            Map<BoltServerAddress, Object> responsesByAddress) {
    //        var addressToProvider = new HashMap<BoltServerAddress, BoltConnectionSource>();
    ...
    //            given(boltConnectionProvider.getConnection(any())).willReturn(completedFuture(boltConnection));
    //
